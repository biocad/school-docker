...
public class Visual extends Applet implements KeyListener, MouseMotionListener {
	...
	public void drawGrid(double[][][] grid, double value) {
		int n = grid.length;
		dark = !dark;
		wrapper.detach();
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				...
				for (int k = 0; k < n; k++) {
					...
					Vector3d vector = new Vector3d(i, j, k);
	...
	public void drawGrid(Grid grid) {
		...
		for (int i = 0; i < len; i++) {
			...
			TransformGroup cur = new TransformGroup();
			Transform3D transform = new Transform3D();
			Vector3d vector = new Vector3d(cell.i, cell.j, cell.k);
			transform.setTranslation(vector);
			cur.setTransform(transform);
			cur.addChild(cube);
			g.addChild(cur);
	...
	public void drawMolecule(Parser p) {
		...
		for (int i = 0; i < len; i++) {
			...
			TransformGroup cur = new TransformGroup();
			Transform3D transform = new Transform3D();
			Vector3d vector = new Vector3d(atom.x, atom.y, atom.z);
			transform.setTranslation(vector);
			cur.setTransform(transform);
			cur.addChild(sphere);
	...
	public Visual(int h) {
		...
		// axises
		Cylinder xAxis = new Cylinder(0.5f, 300);
		everything.addChild(xAxis);

		Cylinder yAxis = new Cylinder(0.5f, 300);
		...
		Cylinder zAxis = new Cylinder(0.5f, 300);
